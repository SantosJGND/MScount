...
def MC_sample_matrix_dict(pop_names, pop_lengths,min_size= 2, samp= [5,20,10], stepup= "increment", diffs= False, frequency_range= [0,1],indfile= 'ind_assignments.txt', ind_ext= False,outemp= 'ind_assignments{}.txt',chrom_idx= 0,
                    ...
                    outlog= 'indy.log', row= 24,col= 4, single= True, exclude= False, print_summ= False, sample_sim= 0,collapsed= True,haps_extract= True, bases= 'ACGT',ksize= 3,ploidy= 2, freq_extract= False, sim_del= 'C',
    ...
    for sim in sim_sub:
        ...
        ### read vcf
        t0= time.time()
        Window, mut_matrix, scale, refseq, subset_summary= VCF_read_filter(
        	sim, sim_dir= sim_dir,chrom= chrom,haps_extract= haps_extract, scale_genSize= scale_genSize,
            collapsed= collapsed,min_size= min_size, samp= samp, stepup= stepup, outemp= outemp, return_sub= True,
            indfile= indfile, diffs= diffs,bases= bases, ksize= ksize, ploidy= ploidy
            )

        mut_idx= np.argmax(mut_matrix,axis= 0)
        
        tag_list, tag_dict, pop_dict= ind_assignment_dict(sim,pop_names,pop_lengths,dir_sim= sim_dir, haps_extract= haps_extract,
                          min_size= min_size, samp= samp, stepup= stepup, outemp= outemp,indfile= indfile,ind_ext= ind_ext)

        total_inds= sum([len(x) for x in pop_dict.values()])
        print(total_inds)
        if not len(Window) or Window.shape[0] != total_inds:
            print('ind number in vcf does not match indfile. ind_ext= {}'.format(ind_ext))
            continue
        ...
        for pop in pop_dict.keys():
            ...
            privec= np.zeros(pop_wind.shape[1])
            privec[private]= 1
            privec= privec == 0

            pop_wind[:,privec]= 0

            Window[klist,:]= pop_wind
        ...
        t1= time.time()
        count_time= t1- t0
        print('count_time: {}'.format(count_time))
        print(len(tag_list))
        ...
        if len(tag_list):
            ...
            ###
            Window_lengths= [0] + list(Window_lengths)
            print(Window_lengths[:10])
            ...
            for idx in range(len(tag_list)):
                ...
                for snp_n in Window_lengths:
                    ...
                    pop_dict= tag_dict[tag]
                    ##
                    pop_summary, dummy= count_popKmers(Window[:,lrange], mut_matrix[:,lrange], mut_idx,pop_dict, single= single, prop_gen_used= prop_gen_used,
                                                                      frequency_range= frequency_range,row=row,col=col,segregating= segregating,scale= scale)
                    data_kmer[new_sim]= pop_summary
