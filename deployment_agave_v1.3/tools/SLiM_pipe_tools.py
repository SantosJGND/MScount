...
def read_chrom_sizes(assembly,size_dir= 'chrom_sizes/',reject= ['M','Un','X','Y','_','A','B']):
    '''
...
def fasta_RextractUnif(fasta,seq_store,L= 10000):
    ...
    for chrom in Outfiles.keys():
        ...
        if len(result) < L:
            print('chom {} len < requested fasta L= {}'.format(chrom,L))
            continue
...
def return_seqs(seq,size= 10,L= 1000,keep= ['A','T','G','C'],threshold= 0.1):
...
def bash_launch(command_here,log_ID,log_dir= '.', wait= 30):
    '''
    deploy script in bash but leaving log and nohup. 
    '''

    log_name= "/".join([log_dir,log_ID]) + ".log"
    command= '{}> {} &'.format(command_here, log_name)

    print(command)

    os.system("""{}""".format(command))
    if wait:
        sleep_order= 'sleep {}s'.format(wait)
        os.system("""{}""".format(sleep_order))
